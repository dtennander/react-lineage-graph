import { Dispatch, ReactElement, createContext, useContext, useMemo, useState, useSyncExternalStore } from "react"
...
export const LineageProvider = ({ children }: { children: ReactElement }) => {
  ...
  return (
    <LineageContext.Provider value={store}>
      {children}
...
export const useSetNode = () => {
  const store = useContext(LineageContext)
  if (!store) throw new Error("useSetNode must be used within a LineageProvider")
